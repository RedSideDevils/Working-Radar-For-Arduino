...
while True:
    ...
    for target in targets:
        ...
        if(target[0] == math.floor(x0 + math.cos(math.radians(moving_line_angle)) * dist)):
            if(dist < r and target[1] <= y0 and moving_line_angle <= 180) or (dist < r and target[1] >= y0 and moving_line_angle >= 180):
    ...
    if(moving_line_angle >= 360):
        moving_line_angle = moving_line_angle - 360
